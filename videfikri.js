        ...
        break
            ...
            case 'stnc':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    await vf.sendImageAsSticker(from, imageBase64, { keepScale: true, author: 'RIZ', pack: 'RIZ BOT' })
                    console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                } catch (err) {
                    console.error(err)
                    await vf.reply(from, 'Error!', id)
            ...
            case 'stiker':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    const mediaData = await decryptMedia(encryptMedia, uaOverride)
                    const imageBase64 = `data:${_mimetype};base64,${mediaData.toString('base64')}`
                    await vf.sendImageAsSticker(from, imageBase64, { author: 'RIZ', pack: 'RIZ BOT' })
                    console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                } catch (err) {
                    console.error(err)
                    await vf.reply(from, 'Error!', id)
            ...
            case 'stikergif':
                ...
                if (isMedia && type === 'video' || mimetype === 'image/gif') {
                    ...
                    try {
                        const mediaData = await decryptMedia(message, uaOverride)
                        const videoBase64 = `data:${mimetype};base64,${mediaData.toString('base64')}`
                        await vf.sendMp4AsSticker(from, videoBase64, { fps: 10, startTime: `00:00:00.0`, endTime : `00:00:06.0`, loop: 0 }, { author: 'RIZ BOT', pack: 'RIZ' })
                            .then(async () => {
                                console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
            ...
            case 'emot':
                ...
                await vf.sendImageAsSticker(from, await vf.download(`https://videfikri.com/api/emojitopng/?emojicode=${emoji}`), { author: 'RIZ', pack: 'RIZ BOT' })
                } catch (err) {
                    console.error(err)
                    await vf.reply(from, 'Error!', id)
                }
